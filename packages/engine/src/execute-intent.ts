...
import { transact, writeDb } from "@console/db-write";
...
function runGoverned(
  ...
  if (policy.effect === "deny") {
    ...
    const auditId = appendAudit(tx, {
      ...
      recordId: record?.id ?? "-",
      event: "denied",
      summary: `${action.label} blocked: ${reason}`,
      payload: input,
      before: null,
      after: null,
      decision: policy,
  ...
  if (policy.effect === "require_approval") {
    ...
    const auditId = appendAudit(tx, {
      ...
      summary: `${decision.summary}. Sent to ${policy.tier ?? "manager"} for approval: ${reason}`,
      payload: input,
      before: null,
      after: null,
