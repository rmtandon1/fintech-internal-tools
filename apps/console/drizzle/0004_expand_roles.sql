...
UPDATE approval_requests SET requester_role = 'kyc_manager' WHERE requester_role = 'manager' AND tool = 'flags';--> statement-breakpoint
-- Keep the demo requester id paired with its remapped role so a later
-- approval is audited under a consistent identity. Runs after the role
-- statements, so ids are derived from the new role values. Historical
-- decided_by ids and audit rows are left alone.
UPDATE approval_requests SET requester_id = 'usr_' || requester_role WHERE requester_id IN ('usr_analyst','usr_manager');
