...
- Tools are workspace packages in `tools/<tool>/` (`src/index.ts`, `src/schema.ts`, `src/seed.ts`, `package.json`), registered in `apps/console/src/registry.ts` and their tables re-exported from `apps/console/src/schema.ts`; a tool that depends on another tool declares it in its `package.json`. Queue stats are declared on the tool (`stats: StatDecl[]`, see `docs/QUEUE_STATS_STRIP.md`) and rendered as stat cards by `apps/console/src/components/stat-strip.tsx`; every count is a query, never a stored number. A tool with an on/off field may declare `toggle` (`ToggleDecl`: the field, the on and off actions, an optional `groupBy`) to open its queue as compact switches (`apps/console/src/components/toggle-grid.tsx`); flipping one runs the action through `executeIntent` like any other
